from Crypto.Util.number import long_to_bytes
from string import ascii_letters

MSGS = (, 0x234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f, 0x32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb, , 0x3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070, , , , , 0x466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83, 0x32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904)
MAX = len(long_to_bytes(max(MSGS)))
MSGS = [long_to_bytes(text) for text in MSGS]
#KEY = [0 for _ in range(MAX)]
KEY = [[] for _ in range(MAX)]
CIPHER = long_to_bytes(0x32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904)

def verify_key(c: int):
    k = c^ord(' ')
    if chr(c^k) == ' ':
        return True
    return False

def find_keys(c1: str, c2: str):
    lower = min(len(c1), len(c2))
    for i in range(lower):
        m1_m2 = int(c1[i])^int(c2[i])
        if (m1_m2 >= 65 and m1_m2 <= 90) or (m1_m2 >= 97 and m1_m2 <= 122):
            KEY[i].append(int(c1[i])^ord(' '))
            KEY[i].append(int(c2[i])^ord(' '))

def two_item_comb(list: list):
    final_list = []
    for i in range(len(list)):
        for j in range(i+1, len(list)):
            final_list.append([list[i],list[j]])
    return final_list

def most_frequent(List):
    counter = 0
    num = List[0]

    for i in List:
        curr_frequency = List.count(i)
        if(curr_frequency> counter):
            counter = curr_frequency
            num = i

    return num

if __name__ == '__main__':
    combinations = two_item_comb(MSGS)
    [find_keys(comb[0], comb[1]) for comb in combinations]
    #print(KEY)
    print(''.join([chr(CIPHER[i]^most_frequent(KEY[i])) for i in range(len(CIPHER))]))

